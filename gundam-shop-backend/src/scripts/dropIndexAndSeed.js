...
const dropIndexAndSeed = async () => {
    try {
        await mongoose.connect(process.env.MONGO_URI);
        console.log('✅ Đã kết nối MongoDB');

        // Drop collection để xóa tất cả indexes
        try {
            await Product.collection.drop();
            console.log('🗑️  Đã xóa collection cũ');
        } catch (err) {
            console.log('ℹ️  Collection không tồn tại, tạo mới');
        }
